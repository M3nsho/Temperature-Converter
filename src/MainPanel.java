...
public class MainPanel extends JPanel {
    ...
    public MainPanel(TemperatureConverterFrame parent) {
        ...
        convertButton.addActionListener(e -> {
            ...
            String fromUnit = ((String) inputUnit.getSelectedItem()).split(" ")[0]; //  بيفصل الكلمة اما يجي سبيس
            String toUnit = ((String) convertUnit.getSelectedItem()).split(" ")[0]; // بيفصل الكلمة اما يجي سبيس

            try {
                double inputValue = Double.parseDouble(inputText);// هنا بحول السترينج ل رقم نوعه دابل
                double convertedValue = TemperatureUtils.convertTemperature(inputValue, fromUnit, toUnit);
                // في السطر اللي فوق دا عملت فاريابل جديد بيتخزن فيه قيمة الحرارة بعد ما بيخش المدخل علي التحويل
                String resultText = String.format("%.2f %s", convertedValue, toUnit);

                // بحول هنا عشان الميثود بتاعت تغيير لون ال result panel
